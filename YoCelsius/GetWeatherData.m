...
- (void)startGetLocationWeatherData {
    ...
    NSString *urlString = [NSString stringWithFormat:@"%@?lat=%@&lon=%@&APPID=%@&lang=zh_cn", baseUrl, latStr, lonStr, appIdKey];
...
- (void)requestSucess:(Networking *)networking data:(id)data {
    ...
    if (networking.tag == kWeather) {
        ...
        if (currentData.cod.integerValue == 200) {
            ...
            NSString *urlString = [NSString stringWithFormat:@"%@?id=%@&cnt=%@&APPID=%@&lang=zh_cn",
